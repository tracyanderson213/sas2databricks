...
dbutils.widgets.dropdown("api_style", "dlt", ["dlt", "dp"], "API Style (dlt or dp)")
...
def post_process_converted_code(code, table_analysis, sas_source_text=""):
    """
    ...
    # GENERATE PROC FORMAT dictionaries + UDFs
    if proc_formats:
    ...
    # GENERATE MERGE join code and replace broken SQL
    if merge_patterns:
        api_style = "dp" if "from pyspark import pipelines as dp" in code else "dlt"
        ...
        for pattern in merge_patterns:
            ...
            correct_code = generate_merge_join_code(pattern, api_style)
    ...
    # GENERATE RETAIN window function code and replace broken logic
    if retain_patterns:
        api_style = "dp" if "from pyspark import pipelines as dp" in code else "dlt"
        ...
        for pattern in retain_patterns:
            ...
            correct_code = generate_retain_window_code(pattern, api_style)

            # Find and replace the broken function
            # FIXED: Same regex improvements as MERGE pattern
    ...
    if 'cast(' in code.lower() and '$' in code:
        # If we have extracted format definitions, replace with UDF calls
        if proc_formats:
            for fmt_name in proc_formats.keys():
            ...
            fixes_applied.append("Replaced SAS format functions with UDF calls")
        else:
    ...
    if fixes_applied or warnings or datalines_parsed:
        ...
        # Show auto-generated tables first (big win!)
...
def generate_inline_dataframe_code(table_name, datalines_info, indent="    "):
    ...
    # Build schema string
    schema_parts = [f"{col} {typ}" for col, typ in zip(columns, types)]
...
def translate_merge_to_join(sas_source_text, converted_code):
    ...
    for match in re.finditer(merge_pattern, sas_source_text, re.IGNORECASE | re.DOTALL):
        ...
        merge_clause = match.group(2)
        by_vars = match.group(3).strip()
        post_merge_logic = match.group(4)
...
def translate_retain_to_window(sas_source_text):
    ...
    for match in re.finditer(retain_pattern, sas_source_text, re.IGNORECASE | re.DOTALL):
        output_table = match.group(1)
        input_table = match.group(2)
        by_vars = match.group(3).strip()
        retain_var = match.group(4).strip()
        retain_init = match.group(5).strip()
        logic = match.group(6)

        # Extract accumulation logic: var + amount or var + expression
        accum_pattern = rf'{retain_var}\s*\+\s*(\w+)'
        accum_match = re.search(accum_pattern, logic)

        if accum_match:
            accum_source = accum_match.group(1)
...
def generate_merge_join_code(merge_pattern, api_style="dp"):
    """
    ...
    code = f'{decorator}(name=\'silver.{output_table}\')\n'
    ...
    for i, table in enumerate(tables):
        ...
        code += f'    {var_name} = spark.read.table("bronze.{table["name"]}")'
    ...
    if len(tables) == 2:
        ...
        if tables[0].get('in_flag') or tables[1].get('in_flag'):
            ...
            if tables[0].get('in_flag'):
                code += f'    result = result.withColumn("{tables[0]["in_flag"]}", \n'
                code += f'        F.when(left_df.{by_vars}.isNotNull(), F.lit(1)).otherwise(F.lit(0))\n'
                code += f'    )\n'
            if tables[1].get('in_flag'):
                code += f'    result = result.withColumn("{tables[1]["in_flag"]}", \n'
                code += f'        F.when(right_df.{by_vars}.isNotNull(), F.lit(1)).otherwise(F.lit(0))\n'
                code += f'    )\n'
...
def generate_retain_window_code(retain_pattern, api_style="dp"):
    """
    ...
    code += f'    window = Window.partitionBy("{partition_by}") \\\n'
    code += f'                   .orderBy("{order_by}") \\\n'
    code += f'                   .rowsBetween(Window.unboundedPreceding, Window.currentRow)\n'
    code += f'    \n'
    code += f'    return spark.read.table("silver.{input_table}") \\\n'
...
def generate_format_dict_code(format_name, format_info):
    ...
    # Generate UDF (use T.StringType() which is imported as 'from pyspark.sql import types as T')
    udf_code = f"""
...
def generate_intelligent_template(
    ...
    transformed_code = post_process_converted_code(converted_code, table_analysis, sas_source_text)
...
print("="*80)
print("🔄 CONVERTING WITH INTELLIGENT LAYER DETECTION")
print("="*80)
print()
...
print()
print("="*80)
print("✅ INTELLIGENT CONVERSION COMPLETE!")
print("="*80)
...
if success_count > 0:
    print("="*80)
    print("✅ SUCCESSFUL CONVERSIONS (with intelligent enhancements)")
    print("="*80)
    print()
