...
#   2. databricks bundle run <setup_job_key>    # creates data/genie/ka/mas,
...
# Defaults: profile from DATABRICKS_CONFIG_PROFILE; latest run of the setup
# job (found by name) in the current bundle target.
set -euo pipefail

# ⚠️ RENAME PER DEMO: substring matched against the setup job's display name to
# find it without needing the bundle's --var values here. Must match the name
# in databricks.yml (resources.jobs.<key>.name) — including any dev-mode prefix
# the bundle adds. e.g. "<Demo> Setup".
SETUP_JOB_NAME_MATCH="<Demo> Setup"
...
# Find the job by display name so we don't need the bundle's --var values here.
if [[ -z "$JOB_ID" ]]; then
...
[[ -n "$JOB_ID" ]] || { echo "[finalize] ERROR: couldn't find the setup job by name ('$SETUP_JOB_NAME_MATCH')." >&2; exit 1; }
...
[[ -n "$RUN_ID" ]] || { echo "[finalize] ERROR: no run found for job $JOB_ID — run the setup job first." >&2; exit 1; }
...
# Env vars the app reads (config/app.json placeholders). __VALUEFROM__ entries
# are platform bindings (warehouse + postgres) and stay as `valueFrom`; plain
# values come from the harvested resources JSON.
# ⚠️ EDIT PER DEMO: keep the binding rows that match your databricks.yml
# apps.<key>.resources bindings, and the plain rows that match your app's
# config/app.json env keys.
env_lines = [
    ("NODE_ENV", "production"),
...
# Keep everything BEFORE the `env:` line (command + user_authorization scopes).
head = text.split("\nenv:", 1)[0].rstrip() + "\n"
