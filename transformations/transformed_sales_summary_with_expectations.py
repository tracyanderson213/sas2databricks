...
# ==============================================================================
# Name:           transformed_sales_summary_with_expectations.py
# Original File:  sales_summary_with_expectations.sas
...
"""
/********************************************
* Sales Summary Report - WITH DATA QUALITY EXPECTATIONS
*
...
DATA work.customers;
    ...
    /* EXPECTATION: Company names should be populated after filter */
    /* EXPECT CompanyName IS NOT NULL AND LENGTH(CompanyName) > 0 */

    /* EXPECTATION: Email should be present for contact */
    /* EXPECT EmailAddress IS NOT NULL */
RUN;
...
PROC SQL;
    ...
    /* EXPECT CompanyName IS NOT NULL */
...
# Source: sales_summary_with_expectations.sas
from pyspark import pipelines as dp  # Apache Spark 4.1+ Declarative Pipelines (SDP)
from pyspark.sql import functions as F

@dlt.expect_or_drop("valid_rows", 'CompanyName IS NOT NULL')
...
def add_bronze_metadata(df, source_file="sales_summary_with_expectations.sas", layer="bronze"):
    """
